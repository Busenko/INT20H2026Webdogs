...
Перед початком переконайтеся, що на вашому комп'ютері **встановлено та запущено Docker Desktop** (або Docker Engine для Linux).
...
Джерело: [gis.ny.gov](https://gis.ny.gov/civil-boundaries)
Ці дані стали основою для визначення юрисдикції за координатами (Latitude/Longitude).
...
Ресурс:  [mapshaper](https://mapshaper.org/)

**Команда для Mapshaper (конвертація та проектування):**

- Ця команда переводить вхідні дані у десяткові градуси (широту та довготу):
```
-proj wgs84 
```
- Конвертує ваші географічні об'єкти (кордони округів чи міст) у текстовий формат JSON:
```
-o format=geojson 
```
---
...
Джерело: [NY State Department of Taxation and Finance](https://www.tax.ny.gov/pdf/publications/sales/pub718.pdf)
