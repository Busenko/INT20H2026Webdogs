...
Конвертація та обробка геоданих Оскільки початкові дані часто надаються у форматі Shapefile (.shp) та в метричній системі координат, було використано інструмент Mapshaper: Ресурс: https://mapshaper.org/
Команда для Mapshaper (конвертація та проектування): -proj wgs84 Ця команда переводить вхідні дані у десяткові градуси (широту та довготу). -o format=geojson Конвертує ваші географічні об'єкти (кордони округів чи міст) у текстовий формат JSON.
